...
for game in games:
	...
	try:
		with open("tango_patches/{}_versions.toml".format(game), 'a', encoding = 'utf-8') as file:
			...
			if "bn6" in game:
				...
				gamever = "exe6"
			append = "[versions.'{}']\n".format(version)
			append += "netplay_compatibility = \"daniel{}v{}\"\n".format(gamever,version)
	...
	txt = base + txt
	folderpath = "tango_patches/{}_daniel/v{}".format(game,version)
	if not os.path.exists(folderpath):
		os.mkdir(folderpath,0o666)

	newfile = open("tango_patches/{}_daniel/info.toml".format(game,version), 'w', encoding = 'utf-8')
	bb = newfile.write(txt)
	newfile.close()

	for name in patchnames[game]:
		shutil.copy("{}/{}".format(game,name),"tango_patches/{}_daniel/v{}/{}".format(game,version,name))
